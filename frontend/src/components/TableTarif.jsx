...
const TarifTable = ({ data, onEdit, onDelete }) => {
	...
	useEffect(() => {
		setFilteredData(
			data.filter((item) =>
				[item.daya, item.tarifperkwh].some((field) =>
					...
					<tbody>
						{filteredData.map((item, index) => (
							...
							>
								...
								<td className="border border-gray-300 px-2 py-2">
									{formatNumber(item.daya)}
								</td>
								<td className="border border-gray-300 px-2 py-2">
									{formatCurrency(item.tarifperkwh)}
								</td>

								<td className="border border-gray-300 px-4 py-2">
									<button
										onClick={() => onEdit(item.id_tarif)}
										className="bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded-lg m-1"
									>
										Edit
									</button>
									<button
										onClick={() => handleDeleteConfirm(item.id_tarif)}
